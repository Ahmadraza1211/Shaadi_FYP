...
export default function CategoryManager() {
  ...
  const addSub = async () => {
    ...
    // REQ D: Admin cannot add subcategories inside the Bridal subcategory.
    // The bridal subcategory is locked; Groom and every other category remain
    // open for new subcategory additions. This mirrors the backend 403 guard.
    if (selected.category_id === 'bridal' || selectedSub?.id === 'bridal') {
      showSubMsg('Cannot add subcategories inside Bridal — Groom and other categories are allowed.');
      return;
    }
  ...
  // REQ D: Bridal is a locked subcategory — admin cannot add nested
  // subcategories when the selected category or subcategory is "bridal".
  // Groom and every other category (including wedding_dress parent) remain
  // open for new subcategory additions.
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <div className="border-t border-gray-100 pt-3 space-y-2">
                  ...
                  <input placeholder="Label (e.g. Sofa Set)" value={newSub.label}
                    ...
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-orange-400 outline-none"
                  />
                  <button onClick={addSub} disabled={bridalLocked}
                    title={bridalLocked ? 'Cannot add subcategories inside Bridal — Groom and other categories are allowed.' : ''}
                  ...
                  {bridalLocked && (
                    <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2">
                      Cannot add subcategories inside Bridal — Groom and other categories are allowed.
                    </p>
                  )}
                  {subMsg && (
