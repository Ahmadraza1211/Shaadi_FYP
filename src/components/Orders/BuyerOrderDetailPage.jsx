    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="bg-white rounded-2xl shadow p-6 mb-4">
        ...
        <div className="text-sm text-gray-600 space-y-1">
          ...
          {/* Subtotal / Shipping / Bank Fee / Total breakdown.
              Backend now stores order.shipping_total at create time, so we
              show it as a separate line item (was previously 0). */}
          <div className="mt-2 border-t border-gray-200 pt-2 space-y-1">
            <div className="flex justify-between">
              <span className="text-gray-600">Subtotal</span>
              <span className="font-mono font-semibold text-gray-800">PKR {(order.subtotal || 0).toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Shipping</span>
              <span className="font-mono font-semibold text-gray-800">PKR {(order.shipping_total || 0).toLocaleString()}</span>
            </div>
            {order.payment_method === "BNPL" && order.bank_processing_fee > 0 && (
              <div className="flex justify-between">
                <span className="text-gray-600">Bank Fee</span>
                <span className="font-mono font-semibold text-amber-700">PKR {(order.bank_processing_fee || 0).toLocaleString()}</span>
              </div>
            )}
            <div className="flex justify-between border-t border-gray-200 mt-1 pt-1">
              <span className="font-bold text-gray-900">Total</span>
              <span className="font-mono font-bold text-gray-900">PKR {(order.total_amount || 0).toLocaleString()}</span>
            </div>
          </div>
      ...
      {/* Packages — only render once the order has moved past
          PENDING_BNPL_APPROVAL (no PKG- exists while the bank is still
          reviewing the BNPL request). */}
