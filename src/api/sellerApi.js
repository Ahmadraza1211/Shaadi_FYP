...
/**
 * Unified search across BOTH retail and thrift storefronts.
 *
 * Calls /api/seller/search WITHOUT the marketplace_type filter, so the Flask
 * backend returns results from both "new" and "thrift" products. Each result
 * retains its own `marketplace_type` field so callers can group / route
 * accordingly.
 *
 * @param {Object} opts
 * @param {string} opts.q       — search query
 * @param {number} [opts.limit] — max results (default 20)
 * @returns {Promise<{success:boolean, products:Array, total?:number}>}
 */
...
export default {
  ...
  // Alias: getMyProducts(sellerId) — convenience positional wrapper around listProducts.
  getMyProducts: (sellerId, opts = {}) => listProducts({ sellerId, ...opts }),
  getPublicProducts,
  getProduct,
  updateProduct,
  deleteProduct,
