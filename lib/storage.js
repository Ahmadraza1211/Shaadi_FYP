...
const CATEGORY_PLACEHOLDER_ROOT = path.join(UPLOAD_BASE, "CategoryPlaceholders");
...
async function saveReviewVoiceAsync(buyerId, productId, buffer, ext = "mp3", agent = "") {
  const safeBuyer = String(buyerId).replace(/[^a-zA-Z0-9_-]/g, "_");
  const safeProduct = String(productId).replace(/[^a-zA-Z0-9_-]/g, "_");
  const safeAgent = String(agent || "voice").replace(/[^a-zA-Z0-9_-]/g, "_");
  const stamp = Date.now().toString(36);
  const publicId = `${safeBuyer}_${safeProduct}_${safeAgent}_${stamp}`;
...
/** Single-file upload for CNIC OCR preview (before full BNPL submit). */
function makeBnplOcrPreviewMiddleware() {
  return bnplMemoryUpload.single("cnic_front");
}
...
function ensureCategoryPlaceholderDir() {
  _ensure(CATEGORY_PLACEHOLDER_ROOT);
  return CATEGORY_PLACEHOLDER_ROOT;
}
...
/** Dowry Estimation / admin category placeholder → Cloudinary folder CategoryPlaceholders */
async function saveCategoryPlaceholderAsync(filename, buffer) {
  const publicId = path.parse(filename).name;
  return _cloudOrLocal("CategoryPlaceholders", filename, buffer, {
    publicId,
    resourceType: "image",
  });
}
...
function makeCategoryPlaceholderUploadMiddleware() {
  return categoryIconMemoryUpload.single("placeholder");
}
...
module.exports = {
  ...
  REVIEW_VOICE_ROOT,
  CATEGORY_ROOT,
  CATEGORY_PLACEHOLDER_ROOT,
  ensureBnplDir,
  ensureOrderDir,
  ensureDisputeDir,
  ensureReviewVoiceDir,
  ensureCategoryDir,
  ensureCategoryPlaceholderDir,
  saveBnplUpload,
  saveBnplUploadAsync,
  saveDisputeUpload,
