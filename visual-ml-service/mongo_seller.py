...
Manages two collections:
  ...
  seller_products  — seller-uploaded products across 6 major categories

seller_products schema (v2 — expanded to all 6 categories)
-----------------------------------------------------------
{
  product_id:          "sp_<uuid>",
  seller_id:           "sel_<uuid>",
  ...
  # Category hierarchy
  major_category:      "wedding_dress | furniture | electronics | kitchen_items | decoration | miscellaneous",
  subcategory:         "string",   # bridal | groom | sofa_set | led_tv | …
  item_type:           "string",   # bridal_lehenga | bridal_sharara | crockery_set | …

  # Kept for backward compatibility + embedding search key
  category:            "string",   # = item_type for wedding_dress, subcategory for others

  # Wedding dress specific
  wedding_dress_type:  "bridal | groom | null",
  color:               "string",
  fabric:              "string",    # Chiffon | Silk | Velvet | Net | Cotton | Other
  embroidery_type:     "string",
  size:                "string",    # S | M | L | XL | Custom

  # General / non-dress
  material:            "string",    # wood type for furniture, steel/non-stick for kitchen
  brand:               "string",    # for electronics, kitchen
  ...
  image_embeddings: [
    { image_id, embedding: [1280 floats] }   # only for wedding_dress
  ],
...
def create_product(
    ...
    # wedding_dress (so ML search can filter by subcategory), else subcategory id.
    compat_category = item_type if (major_category == "wedding_dress" and item_type) else subcategory

    # Auto-calculate discount_price from pct if not provided directly
    if discount_price is None and discount_pct and discount_pct > 0:
        discount_price = round(float(price) * (1 - discount_pct / 100))
    ...
    doc = {
        ...
        "category":            compat_category,   # kept for ML search compatibility
        # wedding dress fields
        "wedding_dress_type":  wedding_dress_type.strip(),
        "color":               color.strip(),
        "fabric":              fabric.strip(),
        ...
        # ML fields (populated after processing)
        "images":              [],
        "image_embeddings":    [],
        "tfidf_vector":        {},
        "keywords":            [],
...
def get_public_products(
    ...
    """Return available products for the buyer marketplace with optional filters."""
    db = _get_db()
    if db is None:
        return {"products": [], "total": 0, "page": page, "limit": limit}
    ...
    sort_spec = sort_map.get(sort_by, sort_map["newest"])
