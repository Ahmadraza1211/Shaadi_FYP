...
async function addSubcategory(req, res) {
  try {
    ...
    const { id, label, custom_fields = [], parent_subcategory } = req.body;

    // Guard: Admin cannot add subcategories INSIDE the "bridal" subcategory.
    // bridal itself is a subcategory of wedding_dress; adding nested
    // sub-subcategories under bridal is forbidden. Allow everything else
    // (including adding to wedding_dress, groom, etc.).
    if (category_id === "bridal" || parent_subcategory === "bridal") {
      return res
        .status(403)
        .json({ success: false, error: "Cannot add subcategories inside Bridal" });
    }
